...
from app.models import (
    Subject, Professor, Classroom, Student, Enrollment,
    Monitor, MonitorAssignment, Feedback, DocumentMetadata
)
...
async def run_massive_tests():
    ...
    for model in [Subject, Professor, Classroom, Student, Enrollment, Monitor, MonitorAssignment, Feedback,
                  DocumentMetadata]:
    ...
    monitor1 = await Monitor(registration="494912", name="Francisco Emilson", email="emilson@ufc.br").insert()
    monitor2 = await Monitor(registration="999999", name="Ana Laura", email="ana@ufc.br").insert()

    assign1 = await MonitorAssignment(weekly_hours=12, monitor=monitor1, classroom=class1).insert()
    assign2 = await MonitorAssignment(weekly_hours=8, monitor=monitor2, classroom=class2).insert()

    # --- Documentos ---
    await DocumentMetadata(original_filename="arvores_avl.pdf", content_type="application/pdf", extension=".pdf",
                           size_bytes=2048, assignment=assign1).insert()
    await DocumentMetadata(original_filename="lichao_tree.pdf", content_type="application/pdf", extension=".pdf",
                           size_bytes=1024, assignment=assign1).insert()
    await DocumentMetadata(original_filename="sql_basico.pdf", content_type="application/pdf", extension=".pdf",
                           size_bytes=512, assignment=assign2).insert()

    # --- Feedbacks ---
    hoje = datetime.now(timezone.utc)
    ontem = hoje - timedelta(days=1)

    await Feedback(registration="hash_111", text="A apostila de AVL é excelente!", rating=5, type=MessageType.ELOGIO,
                   assignment=assign1, created_at=hoje).insert()
    await Feedback(registration="hash_222", text="Tenho dúvida sobre Li-Chao, pode ajudar?", rating=3,
                   type=MessageType.DUVIDA, assignment=assign1, created_at=ontem).insert()
    await Feedback(registration="hash_333", text="O material de SQL está incompleto.", rating=2,
                   type=MessageType.CRITICA, assignment=assign2, created_at=hoje).insert()
    ...
    print(f"   -> Documentos da Alocação 1: {[d.original_filename for d in docs_emilson.items]}")
    ...
    print(f"   -> Busca textual ('excelente'): Encontrou {len(fb_text.items)} feedback(s).")

    # 2. Busca por Monitor (JOIN)
    fb_monitor = await repo_fb.list_by_monitor(monitor1.registration, params)
    print(f"   -> Feedbacks para o Monitor {monitor1.registration}: {len(fb_monitor.items)} encontrados.")

    # 3. Hash do Aluno
    fb_hash = await repo_fb.list_by_student_hash("hash_111", params)
    print(f"   -> Feedbacks do aluno anónimo (hash_111): {[f.text for f in fb_hash.items]}")
    ...
    for item in count_mon.items:
        print(f"      - Monitor: {item.get('monitor_registration')}, Total: {item.get('count')}")
    ...
    for item in count_sub.items:
        print(f"      - Disciplina: {item.get('subject_name')}, Total: {item.get('feedback_count')}")

    # 6. Filtro por Data
    fb_datas = await repo_fb.list_by_date_range(params, start_date=hoje - timedelta(hours=1))
