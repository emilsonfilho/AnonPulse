...
async def test_fluxo_completo_armazenamento_minio():
    ...
    try:
        url_retornada = await storage_service.upload_file(ficheiro_fastapi, nome_ficheiro)

        assert url_retornada is not None
        assert nome_ficheiro in url_retornada

        dados_baixados, content_type_baixado = await storage_service.download_file(nome_ficheiro)
